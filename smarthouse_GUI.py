...
class App(QWidget):
    ...
    def initUI(self):
        ...
        # create bold fonts for title labels
        myFont = QtGui.QFont()
        myFont.setBold(True)
    ...
    def on_button(self, dt):
        ...
        first_start = True
        # if it is not the first launch  hide all the result labels
        if not first_start:
            self.hide_results()

        # if I have not selected default days I set 1
        if (self.days==0):
            self.days=1

        # if I have not selected I default to 1
        if (self.method==0):
            self.method=1
